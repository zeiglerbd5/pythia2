#!/usr/bin/env python3
"""
Categorize spikes from Oct 9-11, 2025 to further validate findings.
"""
...
def generate_categorization_report(db_path: str, output_file: str):
    """Generate spike categorization report for Oct 9-11."""

    conn = sqlite3.connect(db_path)

    # Get spikes from Oct 9-11
    spikes_query = """
    ...
    print(f"Analyzing {len(spikes)} spikes from Oct 9-11, 2025...")
    print()
    ...
    # Generate markdown
    md = []
    md.append("# Spike Type Categorization: Oct 9-11, 2025")
    md.append("")
    md.append(f"Analysis of {len(results)} detected spikes (third validation dataset)")
    md.append("")
    ...
    md.append("## Comparison Across All Periods")
    md.append("")
    md.append("| Category | Oct 9-11 | Oct 12-14 | Oct 15-17 | Oct 18-20 |")
    md.append("|----------|----------|-----------|-----------|-----------|")
    ...
    md.append(f"| Fast & Steep | {fast_steep_pct:.1f}% | 24.2% | 26.3% | 45.2% |")
    md.append(f"| Slow & Large | {slow_large_pct:.1f}% | 42.4% | 36.8% | 31.0% |")
    md.append(f"| Small Mover | {small_mover_pct:.1f}% | 9.1% | 18.4% | 14.3% |")
    md.append(f"| Other | {other_pct:.1f}% | 24.2% | 18.4% | 9.5% |")
    md.append("")
    md.append(f"**Combined Two Types**: {fast_steep_pct + slow_large_pct:.1f}% (Oct 12-14: 66.7%, Oct 15-17: 63.2%, Oct 18-20: 76.2%)")
    md.append("")
    ...
    print(f"\nComparison:")
    print(f"  Fast & Steep: {fast_steep_pct:.1f}% (Oct 12-14: 24.2%, Oct 15-17: 26.3%, Oct 18-20: 45.2%)")
    print(f"  Slow & Large: {slow_large_pct:.1f}% (Oct 12-14: 42.4%, Oct 15-17: 36.8%, Oct 18-20: 31.0%)")
    print(f"  Combined: {fast_steep_pct + slow_large_pct:.1f}% (Oct 12-14: 66.7%, Oct 15-17: 63.2%, Oct 18-20: 76.2%)")


if __name__ == "__main__":
    db_path = "market_data copy_86.db"
    output_file = "SPIKE_CATEGORIZATION_OCT09_11.md"
