#!/usr/bin/env python3
"""
Categorize spikes from Oct 12-14, 2025 to further validate findings.
"""
...
def generate_categorization_report(db_path: str, output_file: str):
    """Generate spike categorization report for Oct 12-14."""

    conn = sqlite3.connect(db_path)

    # Get spikes from Oct 12-14
    spikes_query = """
    ...
    print(f"Analyzing {len(spikes)} spikes from Oct 12-14, 2025...")
    print()
    ...
    # Generate markdown
    md = []
    md.append("# Spike Type Categorization: Oct 12-14, 2025")
    md.append("")
    md.append(f"Analysis of {len(results)} detected spikes (second validation dataset)")
    md.append("")
    ...
    md.append("")
    md.append("| Category | Oct 12-14 | Oct 15-17 | Oct 18-20 |")
    md.append("|----------|-----------|-----------|-----------|")
    ...
    md.append(f"| Fast & Steep | {fast_steep_pct:.1f}% | 26.3% | 45.2% |")
    md.append(f"| Slow & Large | {slow_large_pct:.1f}% | 36.8% | 31.0% |")
    md.append(f"| Small Mover | {small_mover_pct:.1f}% | 18.4% | 14.3% |")
    md.append(f"| Other | {other_pct:.1f}% | 18.4% | 9.5% |")
    md.append("")
    md.append(f"**Combined Two Types**: {fast_steep_pct + slow_large_pct:.1f}% (Oct 15-17: 63.2%, Oct 18-20: 76.2%)")
    md.append("")
    ...
    print(f"\nComparison:")
    print(f"  Fast & Steep: {fast_steep_pct:.1f}% (Oct 15-17: 26.3%, Oct 18-20: 45.2%)")
    print(f"  Slow & Large: {slow_large_pct:.1f}% (Oct 15-17: 36.8%, Oct 18-20: 31.0%)")
    print(f"  Combined: {fast_steep_pct + slow_large_pct:.1f}% (Oct 15-17: 63.2%, Oct 18-20: 76.2%)")


if __name__ == "__main__":
    db_path = "market_data copy_86.db"
    output_file = "SPIKE_CATEGORIZATION_OCT12_14.md"
