#!/usr/bin/env python3
"""
Categorize spikes from Oct 15-17, 2025 to validate findings from Oct 18-20.
"""
...
def generate_categorization_report(db_path: str, output_file: str):
    """Generate spike categorization report for Oct 15-17."""

    conn = sqlite3.connect(db_path)

    # Get spikes from Oct 15-17
    spikes_query = """
    ...
    print(f"Analyzing {len(spikes)} spikes from Oct 15-17, 2025...")
    print()
    ...
    # Generate markdown
    md = []
    md.append("# Spike Type Categorization: Oct 15-17, 2025")
    md.append("")
    md.append(f"Analysis of {len(results)} detected spikes (validation dataset)")
    md.append("")
    ...
    # Comparison with Oct 18-20
    md.append("## Comparison with Oct 18-20")
    md.append("")
    md.append("| Category | Oct 15-17 | Oct 18-20 (original) |")
    md.append("|----------|-----------|---------------------|")
    ...
    md.append(f"| Fast & Steep | {fast_steep_pct:.1f}% | 45.2% |")
    md.append(f"| Slow & Large | {slow_large_pct:.1f}% | 31.0% |")
    md.append(f"| Small Mover | {small_mover_pct:.1f}% | 14.3% |")
    md.append(f"| Other | {other_pct:.1f}% | 9.5% |")
    md.append("")
    md.append(f"**Combined Two Types**: {fast_steep_pct + slow_large_pct:.1f}% (vs 76.2% in Oct 18-20)")
    md.append("")
    ...
    print(f"\nComparison:")
    print(f"  Fast & Steep: {fast_steep_pct:.1f}% (Oct 18-20: 45.2%)")
    print(f"  Slow & Large: {slow_large_pct:.1f}% (Oct 18-20: 31.0%)")
    print(f"  Combined: {fast_steep_pct + slow_large_pct:.1f}% (Oct 18-20: 76.2%)")


if __name__ == "__main__":
    db_path = "market_data copy_86.db"
    output_file = "SPIKE_CATEGORIZATION_OCT15_17.md"
